...
var productAdded =
  "Prekės <span class='new-mini-cart-count'>1</span> vnt. įdėta į krepšelį";
...
if (document.querySelectorAll("#video-dialog-mobile").length > 0) {
  document
    ...
    .addEventListener("click", function () {
      ...
      imageUrl = document
        .querySelector(".product-photo-image img")
        .getAttribute("src");
      productPrice = document.querySelector(
        ".product-content .product-price-default"
      ).innerText;
      productCount = document.querySelector(
        ".product-content #sylius_order_item_quantity"
      ).value;
...
var allAddToCart = document.querySelectorAll(
  ".product-item button[data-handler='addToCart']"
);
...
allAddToCart.forEach(function (item, idx) {
  item.addEventListener("click", function () {
    ...
    imageUrl = productName
      ...
      .getAttribute("src");
    productPrice = productName.querySelector(".product-price-default")
      .innerText;
    productCount = productName.querySelector("#sylius_order_item_quantity")
      .value;
...
function createCart() {
  ...
  document
    .querySelector(".new-mini-cart-ok")
    .addEventListener("click", function () {
      window.location = "/krepselis";
    });
  document
    .querySelector(".new-mini-cart-close")
    .addEventListener("click", function () {
      destroyCart();
    });
  document
    .querySelector(".new-mini-cart-back")
    .addEventListener("click", function () {
      destroyCart();
    });
