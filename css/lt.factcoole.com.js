...
var checkIfNotEmpty = function () {
    if (required(email) && required(phone) && required(firstName) && required(surname)) {
        $('#formStep2 > div.mobile-row > div:nth-child(2) > div.accordion').removeClass('required-accordion');
...
if ($('.accordion-name').find('.fc-form-error').length) {
    $('.accordion-name').addClass('active');
    $('.accordion-name').parent().find('.account-inner').addClass('shown');
}

if ($('.accordion-address').parent().find('.fc-form-error').length) {
    $('.accordion-address').addClass('active');
    $('.accordion-address').parent().find('.account-inner').addClass('shown');
}
