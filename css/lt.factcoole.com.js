...
$(
    '#mask > div.container-fluid > div.main-menu-scroll-container > div > div.main-menu-inner > ul > li:nth-child(5) > ul > li > div.col-xs-100.col-lg-60.submenu-left > div:nth-child(4) > a > div > section'
...
var combiningTest = function () {
    if ($('.nav-displayed').length) {
        if ($('.filters-top').length) {
            $('.filters-top.scrolled').addClass('combined-tests');
        }
    } else {
        if ($('.filters-top').length) {
            $('.filters-top.scrolled').removeClass('combined-tests');
        }
    }
};
